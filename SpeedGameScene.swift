...
class SpeedGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        if gameStarted == false {
            ...
            if let touch = touches.first {
                ...
                squareOne.xSpeed = 2.0
                squareOne.ySpeed = -2.0
    ...
    func loadHighScoreLabel() {
        ...
        highScoreLabel.name = "highScoreLabel3"
    ...
    func loadHighScore() {
        ...
        let highScoreLabel = childNodeWithName("highScoreLabel3") as! ScoreLabel
        highScoreLabel.setTo(defaults.integerForKey("Speed"))
    }
    ...
    func moveSquares() {
        for square in squares {
            ...
            if square.x + (sideLength/2) >= frame.width {
                square.xSpeed = (-1 * CGFloat(abs(square.xSpeed))) - 0.25
                scoreLabel.increment()
            }
            else if square.x - (sideLength/2) <= 0 {
                square.xSpeed = CGFloat(abs(square.xSpeed)) + 0.25
                scoreLabel.increment()
            }
            else if square.y + (sideLength/2) >= frame.height {
                square.ySpeed = (-1 * CGFloat(abs(square.ySpeed))) - 0.25
                scoreLabel.increment()
            }
            else if square.y - (sideLength/2) <= 0 {
                square.ySpeed = CGFloat(abs(square.ySpeed)) + 0.25
                scoreLabel.increment()
            }
    ...
    func endGame() {
        ...
        let pointsLabel = childNodeWithName("pointsLabel3") as! ScoreLabel
        let highScoreLabel = childNodeWithName("highScoreLabel3") as! ScoreLabel
        ...
        if highScoreLabel.number < pointsLabel.number {
            ...
            let defaults = NSUserDefaults.standardUserDefaults()
            defaults.setInteger(highScoreLabel.number, forKey: "Speed")
        }
    }
    
    
    func restart() {
        let newScene = SpeedGameScene(size: view!.bounds.size)
